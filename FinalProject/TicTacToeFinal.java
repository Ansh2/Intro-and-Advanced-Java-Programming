...
public class TicTacToeFinal {
	...
	public static void playGame(Scanner console) {
		...
		while (!isGameDone(board)) {
			if (userplay) {
				userentry(board, console, usersymbol);
			} else {
				computerentry(board, computersymbol);
			}
			printBoard(board);
			userplay = !userplay;
		}
	...
	/*
	 * public static void resetBoard(char[][] board, int size) { if(size <=0)
	 * return; for (int rows = 0; rows < size; rows++) for (int columns = 0; columns
	 * < size; columns++) board[rows][columns] = ' '; printBoard(board); }
	 */
	...
	public static void userentry(char[][] board, Scanner console, char usersymbol) {
		if (boardFilledUp(board)) {
			return;
		}
	...
	public static void computerentry(char[][] board, char computersymbol) {
		if (boardFilledUp(board)) {
			return;
		}
		Random r = new Random();
	...
	public static boolean isGameDone(char[][] board) {
		...
		boolean filled = boardFilledUp(board);
		if (filled) {
			if (winsymbol != ' ') {
				System.out.println("Winner winning symbol:" + winsymbol);
				return true;
			} else {
	...
	/*
	 * Method to check if all spots are filled Parameters: board, two dimentional
	 * char array Returns : true if all spots are filled or false
	 */
	public static boolean boardFilledUp(char[][] board) {
